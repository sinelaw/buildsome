...
import           Lib.Makefile (Makefile(..), TargetType(..), Target, targetAllInputs, TargetDesc, TargetKind)
  ...
  where
      updateCache = {-# SCC "getCachedSubDirHashes.updateCache" #-} do
          ...
          res <-
              if not isDir
              then return Nothing
              else printTimeIt ("Getting full directory info for: " <> show filePath) $ do
...
makeExecutionLog buildsome target inputs outputs stdOutputs selfTime = {-# SCC "makeExecutionLog" #-} do
  ...
  where
    ...
    getExternalSubdirFor path a@(_          , Nothing) = Left (path, a)
    getExternalSubdirFor path a@_ | "/home" `BS8.isPrefixOf` path = Left (path, a)
    getExternalSubdirFor path a@(_, Just stat)
        | Posix.isDirectory stat = Left (path, a)
        | otherwise =
    ...
    externalInputDesc subdir = do
      -- TODO crap
      Just (listingHash, statHash) <- getCachedSubDirHashes buildsome subdir
      return
          $ FileDescExisting
...
buildTarget bte@BuildTargetEnv{..} entity TargetDesc{..} = {-# SCC "buildTarget" #-}
  maybeRedirectExceptions bte TargetDesc{..} $ do
    ...
    case explicitPathsBuilt of
      ...
      ExplicitPathsBuilt | BS8.null $ targetCmds tdTarget ->
